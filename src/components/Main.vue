			...
			<el-row v-if="render">
				...
				<el-col :span="24" :offset="0" v-if="isStart" style="margin-top: 20px;box-shadow: #42B983;">
					<div style="text-align: center;">
						<!-- <el-input
							...
							size="medium"
							disabled
						>{{ nowQt.no }}.{{ nowQt.content }}</el-input> -->
						<!-- <text id="question"
						style="height: 100px;width: 450px;">{{ nowQt.no }}.{{ nowQt.content }}</text>-->
						<textarea disabled="disabled" readonly id="question" style="height: 100px;width: 450px;resize: none;">{{ nowQt.no }}.{{ nowQt.content }}</textarea>
					</div>
				</el-col>
				...
				<el-col :span="12" :offset="9">
					<el-button
						...
						round
						@click="start"
						style="margin-top: 40px;margin-left: 45px;"
					>开 始 问 卷</el-button>
					<el-button
						v-if="isStart"
						type="success"
				...
				<el-row>
					<el-col :offset="14" style="margin-top: 20px;">
						<div>
							...
							<el-divider></el-divider>
							<span>3.完成所有问卷内容后自动提交，请不要立马关闭程序窗口</span>
	...
	methods: {
		...
		async submit() {
			...
			this.recorder.stopRecording(() => {
				...
				fr.onload = _ => {
					this.showSaveDialog(fr.result, fileName)
				}
		...
		showSaveDialog(arrayBuffer, fileName) {
			let that = this;
			// 从本地存储拿保存路径
			let saveFilePath = store.get('saveFilePath')
			if (!saveFilePath) {
				console.log('本地无设置的保存路径')
				saveFilePath = './'
			}
			let path = saveFilePath + '/' + fileName + '.webm'
